  ...
  try {
    ...
    if (existingFriendship) {
      ...
      if (existingFriendship.status === "rejected") {
        await friendshipDAO.delete(existingFriendship.id);
      }
    }
...
export const respondToRequest = async (
  ...
  try {
    ...
    if (friendship.status !== "pending") {
      res
        .status(400)
        .json({ message: "Diese Anfrage wurde bereits beantwortet" });
      return;
    }
    ...
    if (!updated) {
      res
        .status(500)
        .json({ message: "Fehler beim Aktualisieren der Anfrage" });
      return;
    }
...
export const cancelRequest = async (
  ...
  try {
    ...
    if (friendship.status !== "pending") {
      res.status(400).json({
        message: "Diese Anfrage kann nicht mehr zurückgezogen werden",
      });
      return;
    }
